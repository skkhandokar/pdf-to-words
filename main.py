...
async def convert_file(
    ...
    try:
        ...
        # ==========================================
        # ৬. Image to Excel (3-Step Pipeline: Image -> Text -> PDF -> Excel)
        # ==========================================
        elif conversion_type == "image-to-excel":
            output_path = os.path.join(temp_dir, "converted.xlsx")
            ...
            # Step 2: Text to Intermediate PDF
            pdf_path = os.path.join(temp_dir, "intermediate.pdf")
            ...
            # Step 3: PDF to Excel conversion
            all_tables = []
            with pdfplumber.open(pdf_path) as pdf:
                for page in pdf.pages:
                    text_lines = page.extract_text()
                    if text_lines:
                        lines_data = [line.split() for line in text_lines.split('\n') if line.strip()]
                        if lines_data:
                            max_cols = max(len(row) for row in lines_data)
                            normalized_rows = [row + [''] * (max_cols - len(row)) for row in lines_data]
                            df = pd.DataFrame(normalized_rows[1:], columns=normalized_rows[0]) if len(normalized_rows) > 1 else pd.DataFrame(normalized_rows)
                            all_tables.append(df)
            ...
            if all_tables:
                ...
                pd.DataFrame([{"Data": extracted_text}]).to_excel(output_path, index=False)

        else:
            raise HTTPException(status_code=400, detail="Invalid conversion_type specified.")

        # কনভার্ট হওয়া ফাইল ইউজারকে রেসপন্স হিসেবে ডাউনলোড করিয়ে দেওয়া
        return FileResponse(
            path=output_path,
            filename=download_filename,
            media_type="application/octet-stream"
