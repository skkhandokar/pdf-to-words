...
# ১. সিস্টেম ডিপেন্ডেন্সি ইনস্টলেশন (Tesseract OCR, LibreOffice, Poppler-utils এবং অন্যান্য লাইব্রেরি)
RUN apt-get update && apt-get install -y \
    tesseract-ocr \
    libreoffice \
    poppler-utils \
...
# ৪. Render-এর অটোমেটিক PORT অনুযায়ী FastAPI Uvicorn চালুকরণ
CMD ["sh", "-c", "uvicorn main:app --host 0.0.0.0 --port ${PORT:-10000} --workers 1"]